...
const payments: {[id: string]: PaymentRecord} = {};
...
app.post('/payments', (req, res) => {
  const {invoice_id, business_id, amount_cents, payment_method = 'stripe', reference} = req.body;
...
app.get('/payments/:id', (req, res) => {
  const pay = payments[req.params.id];
...
app.get('/businesses/:business_id/payments', (req, res) => {
  ...
  const list = Object.values(payments).filter(p => p.business_id === business_id);
...
app.post('/webhooks/stripe', (req, res) => {
  ...
  if (type === 'charge.succeeded') {
    ...
    // In real system: call /payments POST to record this payment
